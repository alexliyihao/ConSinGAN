...
def ConSinGAN_train(input_name, lr_scale, train_stages, save_directory):
    ...
    opt = parser.parse_args(["--input_name", input_name,
                             "--lr_scale", str(lr_scale),
                             "--train_stages", str(train_stages)])
    opt = functions.post_config(opt)
    opt.save_directory = save_directory
    if opt.fine_tune:
        _gpu = opt.gpu
